...
export default function CheckoutPage() {
  ...
  const validateForm = () => {
    ...
    // Validação de email
    if (!formData.email) {
      newValidations.email = 'Email é obrigatório';
    } else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(formData.email)) {
      newValidations.email = 'Email inválido';
    }

    // Validação de CPF
    if (!formData.cpf) {
      newValidations.cpf = 'CPF é obrigatório';
    ...
    if (paymentMethod === 'credit_card') {
      ...
      if (!formData.cardNumber) {
        newValidations.cardNumber = 'Número do cartão é obrigatório';
      ...
      if (!formData.expiryDate) {
        newValidations.expiryDate = 'Data de expiração é obrigatória';
        ...
        if (expMonth < 1 || expMonth > 12) {
          newValidations.expiryDate = 'Mês inválido';
        } else if (expYear < currentYear || (expYear === currentYear && expMonth < currentMonth)) {
          newValidations.expiryDate = 'Cartão expirado';
        }
      }

      // Validação do CVV
      if (!formData.cvv) {
        newValidations.cvv = 'CVV é obrigatório';
      } else if (formData.cvv.length < 3) {
        newValidations.cvv = 'CVV deve ter pelo menos 3 dígitos';
      }
      ...
      if (!formData.cardName) {
        ...
        newValidations.cardName = 'Nome deve ter pelo menos 3 caracteres';
      } else if (!/^[a-zA-ZÀ-ÿ\s]+$/.test(formData.cardName)) {
        newValidations.cardName = 'Nome deve conter apenas letras';
      }
    }
    ...
    try {
      ...
      if (paymentMethod === 'credit_card') {
        ...
        if (result.success) {
          ...
          try {
            const redirectResponse = await fetch('/api/redirect-after-payment', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({
                paymentStatus: 'paid',
                transactionId: result.transaction_id || 'temp_' + Date.now(),
            ...
            if (redirectData.success) {
              setLoadingMessage(redirectData.redirect.message);
              
              setTimeout(() => {
                setSuccess(true);
              }, redirectData.redirect.delay || 1500);
            } else {
              // Fallback para sucesso mesmo se API de redirect falhar
              setTimeout(() => {
                setSuccess(true);
              }, 1500);
            }
