...
export async function POST(request: NextRequest) {
  try {
    ...
    if (modelsToTrain.length === 0) {
      throw new ApiError('Nenhum modelo válido especificado para treinamento', 400);
    }
    ...
    const retrainJob = {
      job_id: `retrain_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
      status: 'initiated',
    ...
    setTimeout(() => {
      console.log(`Iniciando retreinamento dos modelos: ${modelsToTrain.join(', ')}`);
      // Aqui seria iniciado o processo real de retreinamento
    }, 1000);
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!job_id) {
      // Retornar status geral de retreinamento
      const generalStatus = {
        active_jobs: Math.floor(Math.random() * 3),
        completed_today: Math.floor(Math.random() * 5) + 2,
        queue_length: Math.floor(Math.random() * 10),
        last_successful_retrain: new Date(Date.now() - Math.random() * 7 * 24 * 60 * 60 * 1000).toISOString(),
    ...
    // Simular status de job específico
    const jobStatus = {
      job_id,
