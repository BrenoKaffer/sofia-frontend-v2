...
export default function DashboardPageV3() {
  ...
  const fetchRecentSignals = useCallback(async () => {
    try {
      const token = await getToken();
      const response = await apiClient.getSignalsRecent(undefined, 20);
      ...
      const mappedData = signalsArray.map((signal: any) => ({
        ...signal,
  ...
  const handleSkipPattern = useCallback(() => {
    if (!activeSignal || liveSignalsData.length === 0) return;
    const currentIndex = liveSignalsData.findIndex((s) => s.id === activeSignal.id);
    if (currentIndex === -1) {
      if (liveSignalsData.length > 1) {
        setActiveSignal(liveSignalsData[1]);
      } else {
        setActiveSignal(null);
      }
      return;
    }
    if (currentIndex + 1 < liveSignalsData.length) {
      setActiveSignal(liveSignalsData[currentIndex + 1]);
    } else {
      setActiveSignal(null);
    }
  }, [activeSignal, liveSignalsData]);
              ...
              <ActiveSignalHero 
                ...
                onEnterTable={(tableId) => {
                  setSelectedActiveTable({
                    tableId,
                    strategyName: activeSignal?.strategy_name || 'Estratégia Atual',
                    suggestedBets: activeSignal?.suggested_bets || []
                  });
                }}
                onSkipPattern={handleSkipPattern}
              />
            </div>
          </div>
        </div>
