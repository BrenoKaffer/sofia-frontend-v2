...
const mockRoulettesDisplay: RouletteDataDisplay[] = [
  { id: 'pragmatic-mega-roulette', name: 'Pragmatic Mega Roulette', status: 'online' },
  { id: 'evolution-lightning', name: 'Evolution Lightning Roulette', status: 'online' },
  { id: 'evolution-immersive', name: 'Evolution Immersive Roulette', status: 'online' },
];
...
export function RouletteStatus({ latestSpin, rouletteHistoryData }: RouletteStatusProps) {
  ...
  const [currentRouletteDisplay, setCurrentRouletteDisplay] = useState<RouletteDataDisplay>(mockRoulettesDisplay[0]);

  useEffect(() => {
    if (selectedRouletteIndex < mockRoulettesDisplay.length) {
      setCurrentRouletteDisplay(mockRoulettesDisplay[selectedRouletteIndex]);
    }
    ...
    <Card className="bg-card/50 backdrop-blur-sm border border-border/50">
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-3">
          {mockRoulettesDisplay.map((roulette, index) => (
            <motion.div
              key={roulette.id}
            ...
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className={`w-3 h-3 rounded-full ${getStatusColor(roulette.status)}`} />
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h4 className="font-medium font-heading">{currentRouletteDisplay.name}</h4>
            <Badge variant={currentRouletteDisplay.status === 'online' ? 'default' : 'destructive'}>
              {currentRouletteDisplay.status === 'online' ? 'Online' : 
               currentRouletteDisplay.status === 'offline' ? 'Offline' : 'Manutenção'}
            </Badge>
          </div>

          {/* Current spin (if available via Realtime) */}
          {latestSpin && latestSpin.table_id === currentRouletteDisplay.id ? (
             <motion.div
                key={latestSpin.id}
          ...
          <div>
            ...
            <div className="flex gap-2 overflow-x-auto pb-2">
              {(rouletteHistoryData || [])
                .filter(spin => spin.table_id === currentRouletteDisplay.id)
                .slice(0, 7)
                .map((spin, index) => (
                  <motion.div
                    key={spin.id}
