...
export async function requireAdmin(req: NextRequest): Promise<AuthContext> {
  ...
  // Aqui você pode implementar a lógica de verificação de admin
  // Por exemplo, verificar roles no sistema ou em uma base de dados
  
  // Por enquanto, vamos usar uma lista simples de admin users
  const adminUsers = process.env.ADMIN_USER_IDS?.split(',') || [];
  
  if (!adminUsers.includes(authContext.user.userId)) {
